// Renderer-side wrapper around the codex:* IPC channels.
...
export class CodexRunner {
    ...
    /**
     * @param {string} task
     * @param {{ cwd?: string, approvalMode?: string, binPath?: string,
     ...
     */
    run(task, opts = {}) {
        const api = window.electronAPI?.codex;
        if (!api) return Promise.resolve({ error: 'Codex IPC bridge unavailable. Restart Electron.' });
    ...
    cancel() {
        if (!this._pid || !window.electronAPI?.codex) return;
        window.electronAPI.codex.cancel(this._pid).catch(() => {});
    }
}
